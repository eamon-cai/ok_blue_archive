...
class DailyScheduleTask(BaseBaTask):
    ...
    def do_schedule(self):
        ...
        if self.cycle_time % 10 == 0:
            ...
            if self.target_max_hearts == 0:
                self.logger.warning(f"Can't find any with hearts, probably a bug")
                return False
        ...
        while True:
            ...
            box, count = self.find_most_hearts(hearts)
            if count < self.target_max_hearts:
                self.logger.info(f"Can't find any with target hearts {count} < {self.target_max_hearts}, skip to next")
                self.go_back()
                return True
            self.click_box(box)
            ...
            if dialog_ok.name == "reward_get_ok":
                self.logger.info(f"schedule used all")
                self.set_done()
                return False
    ...
    def find_most_hearts(self, boxes):
        ...
        self.logger.info(f"find_most_hearts size {max_hearts} {max_box}")
